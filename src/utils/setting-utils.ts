...
// Wallpaper image hue follow functions
export function getDefaultFollowImageHue(): boolean {
	return backgroundWallpaper.wallpaperHue?.followImage ?? false;
}

export function getStoredFollowImageHue(): boolean {
	if (
		typeof localStorage === "undefined" ||
		typeof localStorage.getItem !== "function"
	) {
		return getDefaultFollowImageHue();
	}
	const stored = localStorage.getItem("followImageHue");
	if (stored === null) {
		return getDefaultFollowImageHue();
	}
	return stored === "true";
...
export function setFollowImageHue(enabled: boolean): void {
	if (
		typeof localStorage === "undefined" ||
	...
	if (enabled) {
		...
		// 切换为固定色相时，应用配置中的固定色相
		const fixedHue = backgroundWallpaper.wallpaperHue?.fixedHue ?? 250;
		setHue(fixedHue);
	}
	// 通知其他组件
	if (typeof window !== "undefined") {
		window.dispatchEvent(
			new CustomEvent("wallpaperHueChange", {
				detail: { followImage: enabled },
			}),
		);
	}
}
