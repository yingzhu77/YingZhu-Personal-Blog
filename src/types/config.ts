...
export type BackgroundWallpaperConfig = {
	...
	// 壁纸色相跟随配置
	wallpaperHue?: {
		followImage: boolean; // 色相是否跟随壁纸图片变化
		fixedHue: number; // 当followImage为false时的固定色相值 (0-360)
		switchable?: boolean; // 是否允许用户通过控制面板切换此设置
		// 图片名（不含扩展名）到色相的映射，当followImage为true时使用
		imageHueMap?: Record<string, number>;
	};
};
