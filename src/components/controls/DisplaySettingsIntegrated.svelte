...
let followImageHue = $state(getDefaultFollowImageHue());
const defaultFollowImageHue = getDefaultFollowImageHue();
...
onMount(() => {
	const handleHueFollowChange = (event: Event) => {
		const customEvent = event as CustomEvent<{ followImage: boolean }>;
		followImageHue = customEvent.detail.followImage;
	};
    ...
    {#if (wallpaperMode === WALLPAPER_BANNER || wallpaperMode === WALLPAPER_FULLSCREEN) && hasBannerSettings}
        <div class="mt-2 mb-2">
            ...
            <div class="space-y-1">
                ...
                {/if}
                <!-- Hue Follow Image Switch -->
                {#if isHueFollowSwitchable}
                <button
                    class="w-full btn-regular rounded-md py-2 px-3 flex items-center gap-3 text-left active:scale-95 transition-all relative overflow-hidden"
                    class:bg-(--btn-regular-bg-hover)={followImageHue}
                    onclick={toggleFollowImageHue}
                >
...
n                <!-- Random Wallpaper Button -->
                ...
                >
                    <Icon icon="material-symbols:shuffle-rounded" class="text-[1.25rem] shrink-0"></Icon>
                    <span class="text-sm flex-1">{i18n(I18nKey.wallpaperRandom)}</span>
                </button>
